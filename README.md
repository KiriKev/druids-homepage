...
| [`work.html`](work.html) | Archive page — every entry of `latest.json` as a filterable, justified grid (`<body data-page="archive">` switches `portfolio.jsx` to `ArchivePage`) |
| [`latest.json`](latest.json) | Manifest for the **Lately** rail + archive — one line per piece, newest first by `date` |
| [`portfolio.jsx`](portfolio.jsx) | Page composition + `Nav`, `Hero`, `Services`, `Work`, `Latest`, `Approach`, `Tools`, `Builder`, `Testimonials`, `CTA`, `Footer`, lightbox, and the shared `VideoPlayer` |
...
- **Work tile media:** `WORK_MEDIA` array in `portfolio.jsx` — the four curated pieces with editorial copy.
- **Latest feed:** append to [`latest.json`](latest.json); no code or translation changes needed. Bump `?v=` on `LATEST_MANIFEST` in `portfolio.jsx` if a CDN caches it.
- **Copy / translations:** [`i18n.jsx`](i18n.jsx).

## Adding a piece to the Lately rail
...
The home rail shows the newest 10 entries; `work.html` shows all.
